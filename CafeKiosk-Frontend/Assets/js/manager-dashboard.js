(() => {
  ...
  function updateClock(){const now=new Date(); if($('staffDashboardClock'))$('staffDashboardClock').textContent=now.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}); if($('staffDashboardDate'))$('staffDashboardDate').textContent=now.toLocaleDateString([],{weekday:'long',month:'long',day:'numeric',year:'numeric'});}
  function managerSessionName(){
    try {
      const session=JSON.parse(localStorage.getItem('cafeManagerSession')||'null')||window.CafeAuth?.session||{};
      return String(session.displayName||session.fullName||session.full_name||session.name||session.username||'Manager').trim()||'Manager';
    } catch (_) { return 'Manager'; }
  }
  function showSessionName(){if($('staffDashboardName'))$('staffDashboardName').textContent=managerSessionName();}
  function renderRecent(orders,tracking){const body=$('staffRecentOrders');if(!body)return;if(!tracking){body.innerHTML='<tr><td colspan="6" class="staff-empty-cell">Personal order tracking is unavailable for this session. Please use a database-backed Manager account.</td></tr>';return;}if(!orders?.length){body.innerHTML='<tr><td colspan="6" class="staff-empty-cell">No POS orders are linked to your account yet. New POS orders will appear here automatically.</td></tr>';return;}body.innerHTML=orders.map(o=>`<tr><td><span class="staff-order-number">${esc(o.orderNumber)}</span><span class="staff-order-sub">${esc(o.serviceType||'')}</span></td><td>${esc(o.customerName||'Walk-in Customer')}</td><td>${Number(o.itemCount||0)}</td><td><strong>${money(o.total)}</strong></td><td><span class="staff-status-pill ${statusClass(o.status)}">${esc(o.status)}</span></td><td>${esc(time(o.createdAt))}</td></tr>`).join('');}
  function renderStock(items){const wrap=$('staffStockAlerts');if(!wrap)return;if(!items?.length){wrap.innerHTML='<div class="staff-empty-state">No low-stock items right now.</div>';return;}wrap.innerHTML=items.map(i=>`<div class="staff-stock-item ${i.status==='Out of Stock'?'out':''}"><div class="staff-stock-copy"><strong>${esc(i.name)}</strong><small>Alert level: ${Number(i.threshold||0)} ${esc(i.unit||'')}</small></div><div class="staff-stock-level"><strong>${Number(i.currentStock||0)} ${esc(i.unit||'')}</strong><span>${esc(i.status)}</span></div></div>`).join('');}
