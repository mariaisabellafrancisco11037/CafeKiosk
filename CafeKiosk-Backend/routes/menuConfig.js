...
router.put("/",verify,admin,async(req,res,next)=>{try{const cafeId=String(req.user?.cafeId||"cafe-1");const config=await store.put(cafeId,req.body?.config||req.body||{});res.json({success:true,cafeId,config});}catch(e){next(e);}});
