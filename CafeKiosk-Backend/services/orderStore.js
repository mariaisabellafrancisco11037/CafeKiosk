...
async function hydrateRows(rows){
  ...
  return rows.map(r=>({
    id:String(r.order_id),cafeId:r.cafe_id,source:r.source,createdByUserId:r.created_by_user_id?Number(r.created_by_user_id):null,orderNumber:r.order_number,customerName:r.customer_name,customerEligibility:String(r.customer_eligibility||'General').toLowerCase(),serviceType:r.service_type,paymentMethod:r.payment_method,paymentStatus:r.payment_status,cashReceived:num(r.cash_received),change:num(r.change_amount),subtotal:num(r.subtotal),discountAmount:num(r.discount_amount),discount:num(r.discount_amount),total:num(r.total_amount),status:r.status,promotionId:r.promotion_id||undefined,promotionName:r.promotion_name_snapshot||undefined,createdAt:iso(r.created_at),updatedAt:iso(r.updated_at),completedAt:iso(r.completed_at),items:itemMap.get(r.order_id)||[]
  }));
}
...
async function createOrder(order){
  ...
  const conn=await pool.getConnection();try{await conn.beginTransaction();
    ...
    const [r]=await conn.execute(`INSERT INTO orders (order_uuid,cafe_id,order_number,source,created_by_user_id,customer_name,customer_eligibility,service_type,status,promotion_id,promotion_name_snapshot,subtotal,discount_amount,total_amount,payment_method,payment_status,cash_received,change_amount,created_at,updated_at,completed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[
      uuid,order.cafeId,order.orderNumber,order.source,Number.isFinite(Number(order.createdByUserId))?Number(order.createdByUserId):null,order.customerName||'Walk-in Customer',eligibility(order.customerEligibility),order.serviceType||'Dine In',safeStatus(order.status),order.promotionId||null,order.promotionName||null,num(order.subtotal),num(order.discountAmount??order.discount),num(order.total),paymentMethod(order.paymentMethod),paymentStatus(order.paymentStatus),num(order.cashReceived),num(order.change),order.createdAt?new Date(order.createdAt):new Date(),new Date(),order.status==='Completed'?new Date():null
    ]);
